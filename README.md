# 🌿 totocm00's GitHub
<hr style="border: 2px solid black;">

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;![GitHub Nav](https://img.shields.io/badge/Quick-PcNav-black?logo=github)  

⬇︎ ![Public](https://img.shields.io/badge/Public-3CB371?style=for-the-badge&logo=github&logoColor=white)
![Private](https://img.shields.io/badge/Private-DC143C?style=for-the-badge&logo=github&logoColor=white) ⬇   

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[![🧾 Record Hub](https://img.shields.io/badge/🧾_Record_Hub-3CB371?style=for-the-badge&logoColor=white)](https://github.com/totocm00#-record-hub)

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[![🔍 Study: Git & Algorithm](https://img.shields.io/badge/🔍_Study:Git-DC143C?style=for-the-badge&logoColor=white)](https://github.com/totocm00#-study-git--algorithm)

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[![🧭 Project Hub](https://img.shields.io/badge/🧭_Project_Hub-3CB371?style=for-the-badge&logoColor=white)](https://github.com/totocm00#-project-hub)


&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;⬇︎ ![Open Resource](https://img.shields.io/badge/_Open__Resource_-1E90FF?style=for-the-badge&logo=github&logoColor=white)  

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[![🌐 AI & Vision Resources](https://img.shields.io/badge/🌐_AI_&_Vision_Resources-00BFFF?style=for-the-badge&logoColor=white)](https://github.com/totocm00#-ai--vision-resources)

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[![🏷️ Labeling Tools](https://img.shields.io/badge/🏷️_Labeling_Tools_(Detection_Type)-FFD700?style=for-the-badge&logoColor=white)](https://github.com/totocm00#%EF%B8%8F-labeling-tools-by-detection-type)  
 
[![🧰 Dataset Management](https://img.shields.io/badge/🧰_Dataset_Management_&_Visualization-1E90FF?style=for-the-badge&logoColor=white)](https://github.com/totocm00#-dataset-management--visualization-tools)  

[![Notion-Profile](https://img.shields.io/badge/Notion%20Profile-000000?style=for-the-badge&logo=notion&logoColor=white)](https://www.notion.so/Profile-25314a99bcd680379e12caf3168d3a4c?source=copy_link)

---

알고 싶은 것이 많고,  
흩어진 정보와 흐름을 파악해 연결하는 일에 흥미가 있습니다.
  
뛰어난 사람이 되기보다, 
이해를 잘하는 사람이 되고 싶습니다.

30대에는 깊이보다 다양한 경험과 흐름을 통해 시야를 넓히고,  
40대와 50대에는 여러 분야에 대한 전문성을 쌓으며  
새로운 사람들을 키우는 역할까지 해보고 싶습니다.
   
그런 방향의 성장을 꿈꾸는 한 명의 청년입니다.

---
## 🌊 About Me
- 🌿 Intel DX 스마트팩토리 과정 수료 (2025.12.19)

<br>
  
- 🧭 PM/기획 지향
  - 고객사 및 내부 요청사항을 기반으로 요구사항을 구조화하고,  
    목적에 맞는 방향성과 기능 구조를 정의하는 기획 방식 지향
  - 확장 가능성과 재사용성을 고려한 프로젝트 구조 설계 경험
  - 기술 선택 전, 대안 기술 탐색 및 테스트를 통한 적용 가능성 검증
  - 프로젝트 요구에 따라 필요한 기술 스택을 선별·학습·검증하는 방식의 문제 해결
  - 간트차트 및 세부 일정 분해를 통한 일정 관리 및 작업 우선순위 설정 경험

<br>
    
- 🛠 기술 이해 기반 기획 역량  
  - PLC 시퀀스 제어 프로젝트 경험 (GX Works2)
  - 디지털 트윈 연계 경험
     - PLC – OPC – Factory I/O – SCADA 흐름 이해
     - 가상 공정 기반 시나리오 설계 및 검증
  - 로봇·자동화 공정 시나리오 이해

<br>

- 💻 프로그래밍 & 시스템 이해
  - Python
    - 로봇 제어 로직 작성
    - 장비 간 통신 연결 및 데이터 흐름 이해
  - Java
    - 웹/서비스 구조 흐름 이해
    - DB 연동 구조 및 백엔드–프론트엔드 역할 구분 이해
  - Database
    - DB 기본 구조 및 테이블 관계 개념 이해
    - CRUD 동작 흐름 이해
    - View 개념 및 조회 구조 분리 필요성 이해
    - View 구조와 쿼리 성능 저하 이슈를 통합 Optimizer 필요성 인지
    - 물리적 DB 분리(서버/인스턴스/스토리지)와
      논리적 분리(스키마/권한/서비스 단위)에 대한 개념적 이해
    - 트래픽·보안·확장성 관점에서 DB 분리 필요성을 고려한 구조 이

<br>

- 🤝 협업 및 커뮤니케이션
  - 우선순위 판단을 통한 마일스톤 설정 및 이슈 정리
  - 기술·업무 내용을 문서로 정리하여 팀 내 공유한 경험
  - 역할 간(기획-개발-현장) 관점 차이를 고려한 커뮤니케이

<br>

- 📚 업무 도구 활용
  - GitHub 기반 프로젝트 및 문서 관리
  - Google Docs / Sheets, Notion, Canva 활용

<br>
 
- 🚀 기록과 개선 중심의 업무 방식
  - 학습·프로젝트 결과를 문서로 정리
  - 실행 결과를 바탕으로 지속적인 개선 추구

<br>

---


## 📌 Currently Working On

- 📘 커뮤니케이션·협상 역량 강화
  -  『PEERS』 심리학 전공서적을 통한 대화 기술 학습
  -  허브 코웬의 『협상의 기술』을 읽으며 이해관계 조율과 협상 구조 학습

 <br>
   
- 🌍 글로벌 커뮤니케이션 준비
  - TOEIC 학습 진행 중
    - 업무 문서·이메일·기본 회의 이해를 위한 영어 독해 및 청취 역량 강화 목적
  - OPIC 대비 발음·스피킹 기초 학습
    - 실무 커뮤니케이션을 위한 발음 교정 및 말하기 구조 연습
   
<br>
  
- 📜 기획 관점의 지속적 리서치
    - 2026년 대한민국 정부 정책·AI 지원 이슈 지속 모니터링
    - AI 관련 뉴스 및 트렌드를 정리하기 위한 개인 뉴스 수집/정리 구조 기획
 
<br>

- 🌱 산업·사업 관점 탐구
    - 스마트팜 구조 및 현지화 전략, 사업 적용 가능성에 대한 리서치
    - 스마트팩토리가 시장·유통 영역에 적용된 실제 사례 분석 및 시뮬레이션 구상
 
<br>

- 🧩 프로젝트 기획 및 운영 준비
    - 두달살기 부트캠프 QA+기획 파트 참여 시,
       - 역할 정의
       - MVP 프로젝트 기획
       - 일정 관리 및 역할 분담 구조 설계
       - 직군별(개발·디자인·기획) 필요 기술 정리 구상 중
    - 🤝 협업 확장 시도
       - 프로젝트 성격에 맞는 멘토 및 실무자 연결 방식 탐색
       -팀 성장을 위한 외부 인사이트 연계 방안 고민

<br>

---

<br>

## ⚙️ Tech Stack
<details>
  <summary><h5> ▶️ Show Details </h5><sub> ( <눌려주세요. 이미지만 사용 · 링크 아님 )</sub></summary>

### 💻 Language
![Python](https://img.shields.io/badge/Python-4A90E2?style=for-the-badge&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-4A90E2?style=for-the-badge&logo=c&logoColor=white)
![Java](https://img.shields.io/badge/Java-4A90E2?style=for-the-badge&logo=openjdk&logoColor=white)

![SQL](https://img.shields.io/badge/SQL-4A90E2?style=for-the-badge&logo=mysql&logoColor=white)
![JSON](https://img.shields.io/badge/JSON-4A90E2?style=for-the-badge&logo=json&logoColor=white)

![Linux](https://img.shields.io/badge/Linux-4A90E2?style=for-the-badge&logo=linux&logoColor=white)
![Ubuntu](https://img.shields.io/badge/Ubuntu-4A90E2?style=for-the-badge&logo=ubuntu&logoColor=white)

---

### 🧩 Collaboration & Tools
![Git](https://img.shields.io/badge/Git-6C63FF?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-6C63FF?style=for-the-badge&logo=github&logoColor=white)

![Notion](https://img.shields.io/badge/Notion-6C63FF?style=for-the-badge&logo=notion&logoColor=white)
![Obsidian](https://img.shields.io/badge/Obsidian-6C63FF?style=for-the-badge&logo=obsidian&logoColor=white)

![VSCode](https://img.shields.io/badge/VS_Code-6C63FF?style=for-the-badge&logo=visualstudiocode&logoColor=white)
![Eclipse](https://img.shields.io/badge/Eclipse-6C63FF?style=for-the-badge&logo=eclipseide&logoColor=white)
![IntelliJ IDEA](https://img.shields.io/badge/IntelliJ_IDEA-6C63FF?style=for-the-badge&logo=intellijidea&logoColor=white)

![Canva](https://img.shields.io/badge/Canva-6C63FF?style=for-the-badge&logo=canva&logoColor=white)
![PowerPoint](https://img.shields.io/badge/PowerPoint-6C63FF?style=for-the-badge&logo=microsoftpowerpoint&logoColor=white)
![OBS Studio](https://img.shields.io/badge/OBS_Studio-6C63FF?style=for-the-badge&logo=obsstudio&logoColor=white)

---

### ⚙️ Automation / Control
![Mitsubishi PLC](https://img.shields.io/badge/Mitsubishi_PLC-00BFA6?style=for-the-badge&logo=codeforces&logoColor=white)
![GX Works2](https://img.shields.io/badge/GX_Works2-00BFA6?style=for-the-badge&logo=windows-terminal&logoColor=white)

![Siemens PLC](https://img.shields.io/badge/Siemens_PLC-00BFA6?style=for-the-badge&logo=siemens&logoColor=white)
![TIA Portal v5.1](https://img.shields.io/badge/TIA_Portal_v5.1-00BFA6?style=for-the-badge&logo=siemens&logoColor=white)

![HMI–Proface](https://img.shields.io/badge/HMI–Proface-00BFA6?style=for-the-badge&logo=windows-terminal&logoColor=white)
![Arduino](https://img.shields.io/badge/Arduino-00BFA6?style=for-the-badge&logo=arduino&logoColor=white)
![공압실습](https://img.shields.io/badge/공압실습-00BFA6?style=for-the-badge&logo=airplayaudio&logoColor=white)

---

### 🧠 AI / Vision
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6B6B?style=for-the-badge&logo=tensorflow&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-FF6B6B?style=for-the-badge&logo=pytorch&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-FF6B6B?style=for-the-badge&logo=plotly&logoColor=white)

![NumPy](https://img.shields.io/badge/NumPy-FF6B6B?style=for-the-badge&logo=numpy&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-FF6B6B?style=for-the-badge&logo=pandas&logoColor=white)

![OpenVINO](https://img.shields.io/badge/OpenVINO-FF6B6B?style=for-the-badge&logo=intel&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-FF6B6B?style=for-the-badge&logo=opencv&logoColor=white)
![YOLOv5/v8/v11](https://img.shields.io/badge/YOLOv5/v8/v11-FF6B6B?style=for-the-badge&logo=roboflow&logoColor=white)

---

### 🤖 Digital Twin / Simulation / Robot
![NeuroMeka](https://img.shields.io/badge/NeuroMeka-27AE60?style=for-the-badge&logo=robotframework&logoColor=white)
![RoboDK](https://img.shields.io/badge/RoboDK-27AE60?style=for-the-badge&logo=ros&logoColor=white)

![Factory_IO](https://img.shields.io/badge/Factory_IO-27AE60?style=for-the-badge&logo=unity&logoColor=white)
![OPC_UA](https://img.shields.io/badge/OPC_UA-27AE60?style=for-the-badge&logo=modbus&logoColor=white)

<br>
  
***<p align="right"><a href="https://github.com/totocm00#-totocm00s-github">⬆️ Back to Top</a></p>***

</details>

<br>

---

<br>
<br>


### 🧾 Record Hub

- 📊 **AI 정리** : [![AI 정리](https://img.shields.io/badge/AI_Notes-6C63FF?style=for-the-badge&logo=notion&logoColor=white)](https://www.notion.so/AI-2a514a99bcd681238727e9a804cf2c63?source=copy_link)  
  
- 🤖 **협동로봇** : [![협동로봇](https://img.shields.io/badge/Robot_Indy7-007ACC?style=for-the-badge&logo=notion&logoColor=white)](https://www.notion.so/Robot-2a514a99bcd6803bb6cec2c8e4bda989?source=copy_link)     

- 👑 **Ubunto 정리** : [![Ubuntu](https://img.shields.io/badge/Ubuntu_Notes-E95420?style=for-the-badge&logo=notion&logoColor=white)](https://www.notion.so/Ubuntu-2a514a99bcd6814481c6f0c27043b743?source=copy_link)   

- 🚀 **Git 사용방법 정리** : [![GitGuide](https://img.shields.io/badge/GitHub_Guide-000000?style=for-the-badge&logo=notion&logoColor=white)](https://www.notion.so/Git-GitHub-2a514a99bcd680919475d88323c00581?source=copy_link)    

- 🐍 **Python 실습 노트** : [![Python](https://img.shields.io/badge/Python_Study-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/totocm00/python_study)  

- 🧮 **알고리즘 풀이집** : [![Algorithm](https://img.shields.io/badge/Algorithm_Study-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/totocm00/study_algorithm)  

- 📘 **공압(압축공기/실습 노트)** : [![Pneumatics](https://img.shields.io/badge/Pneumatics_Lab-0099CC?style=for-the-badge&logo=notion&logoColor=white)](https://www.notion.so/2a414a99bcd6805391eeff480f0578ce?source=copy_link)  

### 🧾 Record Hub 한방 확인

[![Notion-Profile](https://img.shields.io/badge/Notion%20Profile-000000?style=for-the-badge&logo=notion&logoColor=white)](https://www.notion.so/Profile-25314a99bcd680379e12caf3168d3a4c?source=copy_link)






<br>
<br>

### 🧭 Project Hub

[Notion : AI Projects Introduction](https://www.notion.so/Home-29d9eb4f232780c6be2acbe1b4432c7e?source=copy_link)

- 🏭 AI Projects 0: [open_vision_factory](https://github.com/totocm00/open_vision_factory) / Ongoing Factory
- 🌱 AI Projects 1: [label_text_recognition](https://github.com/totocm00/label_text_recognition) 진행중
- 🌿 AI Projects 2: [조립체 방향 정렬](https://www.notion.so/Home-29d9eb4f232780c6be2acbe1b4432c7e?source=copy_link) / coming soon
- 🌳 AI Projects 3: [PCB 납땜 검사](https://www.notion.so/Home-29d9eb4f232780c6be2acbe1b4432c7e?source=copy_link) / coming soon
- 🦾 Robot : [PLC-Vision-Robot](https://www.notion.so/Home-29d9eb4f232780c6be2acbe1b4432c7e?source=copy_link) / coming soon
- 🔧 PLC : [PLC-OPC-RoboDK : YouTube](https://youtu.be/OVe_I2nfBBo) 


<br>
  
***<p align="right"><a href="https://github.com/totocm00#-totocm00s-github">⬆️ Back to Top</a></p>*** 

  
---

<br>
<br>
  
---

<br>

### 🌿 Study - git

- 📘 [study_algorithm](https://github.com/totocm00/study_algorithm) : 문제풀이 기록용
- 🐍 [python_study](https://github.com/totocm00/python_study) : 기초 문법 학습
- 🌐 [AI_study](https://github.com/totocm00/study_AI) : AI- 트레이닝 저장소
- 🦾 [Robot_study](https://github.com/totocm00/study_AI) : Robot-test / Nuromeka Indy7 모델


***<p align="right"><a href="https://github.com/totocm00#-totocm00s-github">⬆️ Back to Top</a></p>***

  
---
---

<br>


### 🌐 AI & Vision Resources
`인공지능 학습 및 데이터셋 구축에 필요한 주요 프레임워크와 플랫폼 모음`

<br>


<details>
  <summary><b> 🔬 Deep Learning Frameworks</b></summary>  
  
  <br>

- [TensorFlow Keras API](https://www.tensorflow.org/api_docs/python/tf/keras) – 고수준 딥러닝 모델 설계 및 학습용  
- [PyTorch Docs](https://pytorch.org/docs/stable/index.html) – 유연한 연구·실험 중심 프레임워크  
- [OpenVINO GitHub](https://github.com/openvinotoolkit/openvino) – Intel 기반 모델 최적화 및 추론 가속  
- [OpenCV GitHub](https://github.com/opencv/opencv) – 컴퓨터 비전 라이브러리 (이미지·영상 처리 핵심)  
- [YOLOv5 GitHub (Ultralytics)](https://github.com/ultralytics/yolov5) – 대표적인 실시간 객체 탐지 모델  
- [ONNX Runtime](https://github.com/microsoft/onnxruntime) – 다양한 프레임워크 모델 호환 및 경량화 지원

  <br>

</details>

<br>

<details>
  <summary><b> 📊 Dataset & Model Repositories</b></summary>  
  
  <br>

- [Kaggle](https://www.kaggle.com/) – 데이터셋, 경진대회, 커뮤니티 중심 AI 플랫폼  
- [AI Hub (한국)](https://www.aihub.or.kr/) – 한국어 중심 공공 데이터셋 제공  
- [Hugging Face](https://huggingface.co/datasets) – 다양한 언어·비전·음성 데이터셋 및 모델  
- [Roboflow](https://roboflow.com/) – 데이터 업로드, 라벨링, 모델 학습 지원  
- [Google Dataset Search](https://datasetsearch.research.google.com/) – 전 세계 공개 데이터셋 검색엔진  
- [Papers with Code](https://paperswithcode.com/datasets) – 최신 논문과 연결된 벤치마크 데이터셋  

  <br>

</details>

<br>
<br>

### 🏷️ Labeling Tools by Detection Type

<details>
  <summary><b>🧩 Object Detection</b></summary>

  <br>

  - [LabelImg](https://github.com/heartexlabs/labelImg) – 바운딩박스 기반 Object Detection 라벨링  
  - [Makesense.ai](https://www.makesense.ai/) – 브라우저 기반 무료 Detection 툴  
  - [VoTT (Microsoft)](https://github.com/microsoft/VoTT) – 이미지/비디오 라벨링 지원, YOLO/COCO 포맷 내보내기  
  - [Supervisely](https://supervise.ly/) – 협업형 Object Detection 프로젝트 관리  
  - [Label Studio](https://github.com/heartexlabs/label-studio) – 멀티태스크(Detection + OCR) 지원  
  - [Roboflow](https://roboflow.com/) – 업로드, 라벨링, 모델 학습까지 일괄처리 가능한 플랫폼  

  <br>

</details>

<details>
  <summary><b>🎨 Segmentation</b></summary>

  <br> 
  🟢  ·  🔵 


  #### 🟢 Instance / Polygon Segmentation
  - [Labelme](https://github.com/wkentaro/labelme) – 폴리곤 기반 인스턴스 세그멘테이션  
  - [CVAT (Intel)](https://github.com/opencv/cvat) – 다중 객체·프레임 기반 세그멘테이션  
  - [Supervisely Segmentation](https://supervise.ly/) – 협업형 Instance Segmentation 프로젝트  
  - [Scale AI](https://scale.com/) – 상용급 대규모 세그멘테이션 플랫폼  



  <br>

  #### 🔵 Semantic / Panoptic Segmentation
  - [Segment Anything (Meta)](https://github.com/facebookresearch/segment-anything) – SAM 모델 기반 자동 세그멘테이션  
  - [Labelbox](https://labelbox.com/) – 시멘틱 세그멘테이션 및 Class 관리 도구  
  - [CVAT Semantic Mode](https://github.com/opencv/cvat) – Class별 색상 구분형 시멘틱 세그멘테이션  
  - [Supervisely Panoptic](https://supervise.ly/) – 인스턴스+시멘틱 통합형 Panoptic 라벨링  

---
</details>

<details>
  <summary><b>🧾 OCR / Text Detection</b></summary>

  <br>

  - [PaddleOCR](https://github.com/PaddlePaddle/PaddleOCR) – 텍스트 감지 및 인식 프레임워크  
  - [Tesseract OCR](https://github.com/tesseract-ocr/tesseract) – Google의 오픈소스 OCR 엔진  
  - [Label Studio](https://github.com/heartexlabs/label-studio) – OCR / Text Classification 통합형  
  - [DocTR (Mindee)](https://github.com/mindee/doctr) – 문서 내 텍스트 감지 및 인식  
  - [Textract (AWS)](https://aws.amazon.com/textract/) – 클라우드 기반 문서 OCR 자동화  
  - [Kili Technology](https://kili-technology.com/) – OCR, NER, Vision 통합형 Annotation 플랫폼  

</details>

<details>
  <summary><b>🧭 Classification</b></summary>

  <br>

  - [Label Studio](https://github.com/heartexlabs/label-studio) – 이미지 분류(Single/Multi) 지원  
  - [Supervisely Classification](https://supervise.ly/) – 프로젝트 기반 분류형 라벨링  
  - [Roboflow](https://roboflow.com/) – Detection/Classification 통합 관리  
  - [Labelbox](https://labelbox.com/) – 이미지/텍스트/비디오 다중 클래스 분류 지원  
  - [Amazon SageMaker Ground Truth](https://aws.amazon.com/sagemaker/groundtruth/) – 대규모 데이터 분류 자동화  

</details>

<details>
  <summary><b>🧍 Pose / Keypoint / Tracking</b></summary>

  <br>

  - [COCO Annotator](https://github.com/jsbroks/coco-annotator) – Keypoint 기반 포즈 라벨링 및 COCO 포맷 내보내기  
  - [VIA (VGG Image Annotator)](https://www.robots.ox.ac.uk/~vgg/software/via/) – 간단한 포즈/영역 라벨링  
  - [Supervisely Pose Estimation](https://supervise.ly/) – 3D / Keypoint / Tracking 지원  
  - [DeepLabCut](https://github.com/DeepLabCut/DeepLabCut) – 동물 행동/포즈 라벨링용 프레임워크  
  - [OpenPose Annotator](https://github.com/Hzzone/pytorch-openpose) – 포즈 기반 자동 라벨링  

</details>

***<p align="right"><a href="https://github.com/totocm00#-totocm00s-github">⬆️ Back to Top</a></p>***
 

---

<br>

### 🧰 Dataset Management & Visualization Tools

<details>
  <summary><b>📦 Dataset Management & Version Control</b></summary>

  <br>

  - [DVC (Data Version Control)](https://github.com/iterative/dvc) – Git과 유사한 방식으로 데이터 버전 관리  
  - [Git LFS (Large File Storage)](https://git-lfs.github.com/) – 대용량 이미지/비디오 파일 관리용 Git 확장  
  - [LakeFS](https://lakefs.io/) – S3 기반 데이터 레이크 버전관리, ML 파이프라인 통합  
  - [Pachyderm](https://www.pachyderm.com/) – 데이터 이력 추적 및 MLOps 통합 플랫폼  
  - [Quilt Data](https://open.quiltdata.com/) – 데이터 버전 스냅샷 및 시각화 브라우저  

</details>

<details>
  <summary><b>🧮 Dataset Curation & Cleaning</b></summary>

  <br>

  - [Cleanlab](https://github.com/cleanlab/cleanlab) – 잘못된 라벨, 노이즈, 중복 데이터 자동 탐지  
  - [Great Expectations](https://github.com/great-expectations/great_expectations) – 데이터 품질 테스트 및 검증 자동화  
  - [DataPrep](https://github.com/sfu-db/dataprep) – Pandas 스타일 데이터 클리닝 및 분석  
  - [Datasaurus](https://github.com/mwaskom/seaborn-data) – 데이터 시각화 예시셋 관리 (EDA용)  
  - [ActiveLabel](https://github.com/visheratin/activelabel) – Active Learning 기반 라벨링 효율화 툴  

</details>

<details>
  <summary><b>🔍 Dataset Exploration & Visualization</b></summary>

  <br>

  - [FiftyOne (Voxel51)](https://github.com/voxel51/fiftyone) – 이미지/비디오/Detection 결과 시각화 및 필터링  
  - [Weights & Biases (W&B)](https://wandb.ai/site) – 실험 로그 + 데이터셋 시각화 + 협업 대시보드  
  - [ClearML](https://clear.ml/) – 데이터셋 관리, 모델 실험 추적, MLOps 통합  
  - [Comet ML](https://www.comet.com/) – 데이터셋과 실험을 동시 관리 가능한 대시보드  
  - [Gradio + Hugging Face Hub](https://huggingface.co/spaces) – 모델/데이터 브라우징 및 배포 시각화  

</details>

<details>
  <summary><b>🧠 AutoML & Active Learning Integration</b></summary>

  <br>

  - [Label Studio + Active Learning](https://labelstud.io/guide/active_learning) – 라벨링 효율 향상용 반복 학습 구조  
  - [Snorkel AI](https://snorkel.ai/) – 라벨 자동화 및 Weak Supervision 데이터 생성  
  - [Prodigy](https://prodi.gy/) – Active Learning 기반 데이터 라벨링·모델 피드백 통합  
  - [Supervisely Neural Network Integration](https://supervise.ly/) – 학습 결과 기반 라벨 보정  
  - [Lightly.ai](https://www.lightly.ai/) – 비슷한 이미지 클러스터링 및 데이터 선택 최적화  

</details>

<details>
  <summary><b>☁️ Cloud & Enterprise Data Platforms</b></summary>

  <br>

  - [Google Vertex AI Dataset](https://cloud.google.com/vertex-ai/docs/datasets) – 구글 클라우드 기반 통합 데이터 관리  
  - [AWS SageMaker Ground Truth](https://aws.amazon.com/sagemaker/groundtruth/) – 라벨링 + 데이터 버전관리 통합  
  - [Azure ML Datasets](https://learn.microsoft.com/en-us/azure/machine-learning/) – Azure 기반 데이터 관리 플랫폼  
  - [Databricks](https://www.databricks.com/) – 대규모 데이터·레이크하우스 기반 ML 데이터 관리  
  - [Hugging Face Datasets](https://huggingface.co/datasets) – 공개형 ML/DL 데이터셋 카탈로그  
  - [Kili Technology](https://kili-technology.com/) – Annotation + Dataset Versioning 통합 솔루션  

</details>


***<p align="right"><a href="https://github.com/totocm00#-totocm00s-github">⬆️ Back to Top</a></p>***



---

<br>

## 🌼 Contact
> 궁금한 점이 있다면 언제든지 DM 주세요!  

<br>

<br>


## 📊 GitHub Stats ⭐🕒🔀

[![GitHub Streak](https://streak-stats.demolab.com?user=totocm00&theme=dark&border_radius=5&locale=ko)](https://git.io/streak-stats)

<sub>💡 Private 포함 잔디 활성화</sub>

<br>

***<p align="right"><a href="https://github.com/totocm00#-totocm00s-github">⬆️ Back to Top</a></p>***

---

<br>
<br>

## 🌼 Contact
> 궁금한 점이 있다면 언제든지 DM 주세요!  

<br>
<br>


<!--
 ✨ _special_ ✨
- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
